...
const EMOJIS     = ['🌯', '🥙', '🍕', '🍟', '🥖', '🥤', '💧', '🍗', '🥗', '🍖', '🧆', '🫔']
const EMPTY_FORM = { name: '', description: '', price: '', category: 'kebab', emoji: '🌯', tag: '', available: true, image_url: '' }
...
function OptionGroupEditor({ groups, onChange }) {
  const addGroup = () => onChange([...groups, {
    id: `group_${Date.now()}`, label: '', required: false, max_selections: 1, options: []
  }])
  ...
  return (
    <div className={styles.optionGroupEditor}>
      ...
      {groups.length === 0 && <p className={styles.ogEmpty}>No option groups — item goes straight to cart on click.</p>}
          ...
          <div className={styles.ogOptions}>
            {group.options.map((opt, oIdx) => (
              <div key={oIdx} className={styles.ogOption}>
                <input value={opt.label} onChange={e => updateOption(gIdx, oIdx, 'label', e.target.value)} placeholder="Option name" className={styles.ogOptionInput} />
                <div className={styles.ogPriceWrap}>
                  <span>+$</span>
                  <input type="number" min="0" step="0.50" value={opt.price_add} onChange={e => updateOption(gIdx, oIdx, 'price_add', parseFloat(e.target.value) || 0)} className={styles.ogPriceInput} />
                </div>
...
function ItemForm({ initial, onSave, onCancel, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.itemForm}>
      ...
      <div className={styles.field}>
        <label>Photo URL <span style={{ opacity: 0.5 }}>(optional)</span></label>
        <input
...
function AdminPanel({ token, onLogout }) {
  ...
  const handleAdd = async payload => {
    ...
    try {
      ...
      await loadMenu(); setMode('list'); showToast('✅ Item added successfully')
  ...
  const handleEdit = async payload => {
    ...
    try {
      ...
      await loadMenu(); setMode('list'); setEditing(null); showToast('✅ Item updated successfully')
    } catch (e) { setError(e.message) } finally { setSaving(false) }
  }
  ...
  const handleDelete = async item => {
    ...
    try {
      ...
      await loadMenu(); showToast(`🗑️ Deleted: ${item.name}`)
    } catch (e) { setError(e.message) }
  }

  const filtered = filter === 'all' ? items : items.filter(i => i.category === filter)

  // ── Add / Edit screens (full page, no tabs) ──
  if (mode === 'add') return (
    <main className={styles.page}>
      <div className={styles.pageHeader}><h1>Add New Item</h1></div>
  ...
  if (mode === 'edit' && editing) return (
    <main className={styles.page}>
      <div className={styles.pageHeader}><h1>Edit: {editing.name}</h1></div>
      <ItemForm
        initial={{ ...editing, price: editing.price.toString(), tag: editing.tag || '', image_url: editing.image_url || '' }}
        onSave={handleEdit}
    ...
    <main className={styles.page}>
      ...
      <div className={styles.tabNav}>
        {[
          ...
          <button
            key={tab.id}
            className={`${styles.tabNavBtn} ${activeTab === tab.id ? styles.tabNavActive : ''}`}
      ...
      {/* Hours tab */}
      {activeTab === 'hours' && <AdminHours token={token} />}
      ...
      {activeTab === 'menu' && (
        <>
          <div className={styles.menuTabHeader}>
              ...
              {filtered.map(item => (
                <div key={item.id} className={`${styles.itemCard} ${!item.available ? styles.itemUnavailable : ''}`}>
                  <div className={styles.itemCardTop}>
                    {item.image_url
                      ? <img src={item.image_url} alt={item.name} className={styles.itemThumb} />
                  ...
                  <div className={styles.itemActions}>
                    <button className={styles.actionBtn} onClick={() => { setEditing(item); setMode('edit') }}>✏️ Edit</button>
                    <button className={styles.actionBtn} onClick={() => handleToggle(item)}>
                      {item.available ? '🔴 Hide' : '🟢 Show'}
                    </button>
                    <button className={`${styles.actionBtn} ${styles.deleteBtn}`} onClick={() => handleDelete(item)}>🗑️ Delete</button>
                  </div>
                </div>
              ))}
            </div>
